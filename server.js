...
function cashAccounts() {
  ...
  let hk = Number(getSetting("cashHk", "NaN"));
  ...
  if (!Number.isFinite(hk)) {
    hk = 0;
    setCashAccount("港股", hk);
  }
  if (!Number.isFinite(ashare)) ashare = 0;
  if (!Number.isFinite(us)) us = 0;
  const totalCny = Number((ashare + us + (hk * HKD_FX)).toFixed(2));
  setSetting("cash", totalCny);
  return {
    ashare: { market: "A股", currency: "CNY", amount: Number(ashare.toFixed(2)), amountCny: Number(ashare.toFixed(2)) },
    hk: { market: "港股", currency: "HKD", amount: Number(hk.toFixed(2)), amountCny: Number((hk * HKD_FX).toFixed(2)) },
    us: { market: "美股", currency: "CNY", amount: Number(us.toFixed(2)), amountCny: Number(us.toFixed(2)) },
    totalCny
  };
}

function setCashAccount(market, amount) {
  const key = market === "美股" ? "cashUs" : market === "港股" ? "cashHk" : "cashAshare";
...
function changeCashAccount(market, delta) {
  ...
  const current = market === "美股" ? accounts.us.amount : market === "港股" ? accounts.hk.amount : accounts.ashare.amount;
...
function cashAccountForMarket(market) {
  const accounts = cashAccounts();
  return market === "美股" ? accounts.us.amount : market === "港股" ? accounts.hk.amount : accounts.ashare.amount;
}
...
function adjustCashAccount(input) {
  const marketInput = String(input.market || "").toUpperCase();
  const market = marketInput === "US" || input.market === "美股" ? "美股" : marketInput === "HK" || input.market === "港股" ? "港股" : "A股";
  const currency = market === "美股" ? "CNY" : market === "港股" ? "HKD" : "CNY";
...
function portfolioMarketValue(rows) {
  return Number(rows.reduce((sum, item) => sum + currencyToCny(Number(item.qty) * Number(item.price), item.currency), 0).toFixed(2));
}
...
function normalizeHolding(input) {
  ...
  if (!["CNY", "HKD", "USD"].includes(holding.currency)) throw new Error("币种必须是 CNY、HKD 或 USD");
  return holding;
}
...
function normalizeTransactionRecord(input, existing = {}) {
  ...
  const marketCode = marketInput.toUpperCase();
  const market = ["US", "USA", "美股"].includes(marketCode) || marketInput === "美股"
    ? "美股"
    : ["HK", "HKG", "港股"].includes(marketCode) || marketInput === "港股" ? "港股" : "A股";
  const currency = String(input.currency || existing.currency || (market === "美股" ? "USD" : market === "港股" ? "HKD" : "CNY")).trim().toUpperCase();
  ...
  if (!["CNY", "HKD", "USD"].includes(record.currency)) throw new Error("币种必须是 CNY、HKD 或 USD");
  return record;
}
...
function applyTransactionToPortfolio(record, direction = 1) {
  ...
  const fee = Number(trade.fee || 0);
  const cashFactor = trade.market === "美股" ? FX : 1;
  const cashTradeValue = tradeValue * cashFactor;
  const cashFee = fee * cashFactor;
  let cash = cashAccountForMarket(trade.market);

  if (trade.side === "BUY") {
    if (direction > 0) {
      upsertHoldingFromTrade(trade, qty, Number(trade.price));
      cash -= cashTradeValue + cashFee;
    } else {
      upsertHoldingFromTrade(trade, -qty, Number(trade.price));
      cash += cashTradeValue + cashFee;
    }
...
function applyTrade(input) {
  ...
  if (trade.side === "BUY") {
    ...
    cash -= (tradeValue + trade.fee) * cashFactor;
  } else {
    ...
    cash += ((sellQty * trade.price) - trade.fee) * cashFactor;
    trade.qty = sellQty;
  }
